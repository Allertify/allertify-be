...
export async function createUser(input: {
  ...
  try {
    ...
    console.log(
      `[OTP] send to ${email}: ${otp} (valid until ${otpExpired.toISOString()})`,
    );
  } catch (emailError) {
  ...
  return {
    status: existing ? ('OTP_RESENT' as const) : ('REGISTERED' as const),
  };
}
...
export async function verifyOtpAndIssueToken(input: {
  ...
  try {
    ...
    if (freePlan) {
      await subscriptionService.createOrUpgradeSubscription(user.id, freePlan.id, 1);
    }
  } catch (e) {
 ...
 */
export async function getPublicAllergens(
